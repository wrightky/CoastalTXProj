...
`coastx` is a simple Python package for reprojecting latitude and longitude coordinates near the Texas coast into a linear, along-coast reference frame.
...
For some (N,2) array `degree_coordinates`, which contains `longitude` in the first column and `latitude` in the second, convert to along-coast pseudo-coordinates as follows:
```
import coastx
